...
// === HOMEWORK: New service methods ===

// GetStats returns asset statistics (Bài 1)
func (s *AssetService) GetStats() (*model.AssetStats, error) {
	return s.storage.GetStats()
}

// CountByFilter counts assets with optional filters (Bài 1)
...
// BatchCreateAssets creates multiple assets at once (Bài 2)
// Validates ALL assets first, then creates them in a single transaction
func (s *AssetService) BatchCreateAssets(items []struct{ Name, Type string }) ([]*model.Asset, error) {
	// Check batch limit
	if len(items) > 100 {
	...
	assets := make([]*model.Asset, len(items))
	now := time.Now()
...
// BatchDeleteAssets deletes multiple assets by IDs (Bài 3)
...
// ListAssetsWithPagination returns paginated assets (Bài 6)
func (s *AssetService) ListAssetsWithPagination(page, limit int, assetType, status string) (*model.PaginatedResponse, error) {
	// Defaults
	if page <= 0 {
		page = 1
	}
	...
	assets, total, err := s.storage.ListWithPagination(page, limit, assetType, status)
	if err != nil {
		return nil, err
	}
